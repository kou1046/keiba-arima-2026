...
# 「このレースの出走馬は、そのレース開催年の全走行記録も取る」対象 (友人提案)。
# backfill_horses は今日から数年窓なので過去年の出走馬の同年成績を拾えない。その穴埋め用。
# name LIKE で部分一致。本命は有馬記念、他 G1 を足したくなったらここに追加。
TARGET_RACE_NAMES = ("有馬記念",)
